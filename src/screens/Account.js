...
export default function AccountScreen() {
    ...
    return (_jsxs(Screen, { className: s.screen, children: [_jsxs("div", { className: s.top, children: [_jsx(SaveUniverse, {}), _jsx("h1", { className: s.title, children: "Your 777 journey starts here" }), _jsx("p", { className: s.body, children: "Make time for each other, again and again \u2014 with little rituals that keep you close, connected, and in love." })] }), mode === 'choose' ? (_jsxs("div", { className: s.actions, children: [_jsxs(Button, { variant: "secondary", size: "lg", block: true, disabled: busy, onClick: () => void run(() => signInWithProvider('google', next)), children: [_jsx("span", { className: s.mark, "aria-hidden": true, children: _jsx(GoogleMark, {}) }), "Continue with Google"] }), _jsx(Button, { variant: "accent", size: "lg", block: true, onClick: () => setMode('email-up'), children: "Continue with email" }), _jsx("button", { type: "button", className: s.link, onClick: () => setMode('email-in'), children: "Already have an account? Log in" })] })) : (_jsxs("form", { className: s.form, onSubmit: (e) => {
                    e.preventDefault();
                    void run(async () => {
                        if (mode === 'email-up') {
                ...
                }, children: [_jsx(Input, { label: "Email", type: "email", inputMode: "email", autoComplete: "email", value: email, onChange: (e) => setEmail(e.target.value), required: true }), _jsx(Input, { label: "Password", type: "password", autoComplete: mode === 'email-up' ? 'new-password' : 'current-password', value: password, onChange: (e) => setPassword(e.target.value), hint: mode === 'email-up' ? 'At least six characters.' : undefined, required: true }), error ? _jsx("p", { className: s.error, children: error }) : null, _jsx(Button, { type: "submit", variant: "accent", size: "lg", block: true, disabled: busy, children: busy ? 'One moment…' : mode === 'email-up' ? 'Create my account' : 'Sign in' }), _jsx("button", { type: "button", className: s.link, onClick: () => setMode(mode === 'email-up' ? 'email-in' : 'email-up'), children: mode === 'email-up' ? 'Already have an account? Log in' : 'Create an account instead' }), _jsx("button", { type: "button", className: s.link, onClick: () => setMode('choose'), children: "Back" })] })), mode === 'choose' && error ? _jsx("p", { className: s.error, children: error }) : null, _jsx("p", { className: s.fine, children: "Couple777 is a private space for two. Nothing you write here is public." })] }));
}
