...
export function TabBar() {
    ...
    const home = pathname === '/';
    return (_jsxs("nav", { className: s.bar, "aria-label": "Main", children: [_jsx("div", { className: s.side, children: LEFT.map((t) => tab(t)) }), _jsxs("div", { className: s.centre, children: [_jsx(NavLink, { to: "/", className: [s.home, home ? s.homeOn : ''].filter(Boolean).join(' '), "aria-current": home ? 'page' : undefined, "aria-label": "Home", children: _jsx(IconHomeSolid, {}) }), _jsx("span", { className: [s.homeLabel, home ? s.homeLabelOn : ''].filter(Boolean).join(' '), children: "Home" })] }), _jsx("div", { className: s.side, children: RIGHT.map((t) => tab(t, t.to === '/us' && usDot)) })] }));
}
