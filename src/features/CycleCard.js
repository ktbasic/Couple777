...
import { CYCLE_NOUN, cycleHeadline } from '@/lib/cycles';
...
export function CycleCardHero({ view }) {
    ...
    const plan = view.plan;
    const isHidden = hidden(view, me.id);
    return (_jsxs("div", { className: `${s.card} ${s.hero}`, "data-tier": view.cycle.tier, children: [_jsxs("div", { className: s.heroTop, children: [_jsxs("span", { className: s.heroCadence, children: ["Every ", meta.cadence] }), _jsx(ProgressRing, { progress: view.progress, size: 40, stroke: 3, className: s.ring, children: _jsx("span", { className: s.ringNum, children: view.overdue ? '·' : Math.max(0, view.daysAway) }) })] }), _jsx("p", { className: s.heroHeadline, children: cycleHeadline(view) }), plan ? (_jsxs(_Fragment, { children: [_jsxs(Link, { to: `/plan/${plan.id}`, className: s.heroPlan, children: [_jsx("span", { className: s.heroEmoji, "aria-hidden": true, children: isHidden ? '🎁' : plan.emoji }), _jsxs("div", { className: s.heroPlanMain, children: [_jsx("p", { className: s.heroPlanTitle, children: isHidden ? 'A surprise, from them' : plan.title }), _jsxs("p", { className: s.heroPlanMeta, children: [formatPlanDate(plan.date), plan.time ? ` · ${plan.time}` : '', !isHidden && plan.place ? ` · ${plan.place}` : ''] })] })] }), _jsxs("div", { className: s.actions, children: [_jsx(StatusChip, { view: view }), view.status === 'planned' && !isHidden ? (_jsxs(ButtonLink, { to: `/plan/${plan.id}?ask=1`, variant: "accent", size: "sm", children: ["Ask ", partner.name, " \uD83D\uDC8C"] })) : (_jsx(ButtonLink, { to: `/plan/${plan.id}`, variant: "secondary", size: "sm", children: "See plan" }))] })] })) : (_jsxs(_Fragment, { children: [_jsx("p", { className: s.heroEmpty, children: meta.hint }), _jsxs("div", { className: s.actions, children: [_jsx(ButtonLink, { to: `/explore?cycle=${view.cycle.id}`, variant: "accent", size: "sm", children: "Find an idea" }), _jsx(ButtonLink, { to: `/plan/new?cycle=${view.cycle.id}`, variant: "quiet", size: "sm", children: "Create my own" })] })] }))] }));
}
export { CYCLE_NOUN };
