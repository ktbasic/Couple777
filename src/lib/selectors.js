import { byAttention, viewOpenCycles } from './cycles';
...
/* ------------------------------- Memories -------------------------------- */
export function sortedMemories(memories) {
    return [...memories].sort((a, b) => b.date.localeCompare(a.date));
}
export function memoriesByMonth(memories) {
...
/* --------------------------------- Notes --------------------------------- */
export function inboxNotes(state, meId) {
    const now = Date.now();
    return state.notes.filter((n) => n.from !== meId &&
        n.kind !== 'private' &&
        (!n.deliverAt || new Date(n.deliverAt).getTime() <= now));
}
export function scheduledNotes(state, meId) {
    const now = Date.now();
    return state.notes.filter((n) => n.from === meId && n.deliverAt && new Date(n.deliverAt).getTime() > now);
}
export function sentNotes(state, meId) {
    const now = Date.now();
    return state.notes.filter((n) => n.from === meId &&
        n.kind !== 'private' &&
        (!n.deliverAt || new Date(n.deliverAt).getTime() <= now));
}
export function privateNotes(state, meId) {
    return state.notes.filter((n) => n.from === meId && n.kind === 'private');
}
export function unreadCount(state, meId) {
...
export function dailyStatus(state, meId, partnerId, date = today()) {
...
export function relationshipStats(state) {
    ...
    const count = (t) => done.filter((c) => c.tier === t).length;
    return {
 ...
 */
export function notifications(state, meId, partnerId, now = today()) {
    const items = [];
...
export function memoryYear(state, now = today()) {
    const out = [];
