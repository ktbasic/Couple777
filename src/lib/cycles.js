...
export function createInitialCycles(start = today()) {
...
export function openCycle(cycles, tier) {
    return cycles
        .filter((c) => c.tier === tier && !c.completedAt)
        .sort((a, b) => a.dueDate.localeCompare(b.dueDate))[0];
}
export function openCycles(cycles) {
    return TIERS.map((t) => openCycle(cycles, t)).filter((c) => Boolean(c));
}
export function cycleById(cycles, id) {
 ...
 */
export function completeCycle(cycles, cycleId, on = today()) {
    ...
    const alsoClosed = SATISFIES[target.tier]
        ...
        .filter((c) => Boolean(c) && c.id !== target.id);
    const closed = [target, ...alsoClosed];
    const closedIds = new Set(closed.map((c) => c.id));
    const updated = cycles.map((c) => closedIds.has(c.id)
        ? {
            ...c,
            completedAt: at,
...
/* --------------------------------- Status -------------------------------- */
export function cycleStatus(cycle, plan) {
    if (cycle.completedAt)
        return 'completed';
    if (!plan)
        return 'upcoming';
    if (plan.invite?.response === 'yes')
        return 'confirmed';
    if (plan.invite?.sentAt)
        return 'invited';
    return 'planned';
}
...
export function viewCycle(cycle, plans, now = today()) {
...
export function viewOpenCycles(cycles, plans, now = today()) {
 ...
 */
export function attentionScore(view) {
...
export function byAttention(views) {
    return [...views].sort((a, b) => attentionScore(a) - attentionScore(b));
}
/* --------------------------------- Copy ---------------------------------- */
export const CYCLE_NOUN = {
    day: 'moment together',
...
export function listOut(items) {
    if (items.length < 2)
        return items[0] ?? '';
    return `${items.slice(0, -1).join(', ')} and ${items[items.length - 1]}`;
}
/** Gentle, never a scold. */
export function cycleHeadline(view) {
    const noun = CYCLE_NOUN[view.cycle.tier];
    if (view.overdue)
        return `Life got busy. Ready for your next ${noun}?`;
    if (view.daysAway === 0)
        return `Your ${noun} is today`;
    if (view.daysAway === 1)
        return `Your ${noun} is tomorrow`;
