 ...
 */
export function suggestionsFor(note, place, partnerName) {
    const t = note.toLowerCase();
    const out = [];
    const plan = (title, where) => `/plan/new?title=${encodeURIComponent(title)}${where ? `&place=${encodeURIComponent(where)}` : ''}`;
    if (place === 'Home') {
