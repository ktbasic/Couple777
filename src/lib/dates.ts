import type { ISODate, RitualTier } from './types';
...
/** "Saturday · Sep 5" */
export function formatPlanDate(iso: ISODate): string {
  const d = fromISODate(iso);
...
export function formatStamp(iso: ISODate): string {
  const d = fromISODate(iso);
  return `${MONTHS_SHORT[d.getMonth()].toUpperCase()} ${d.getDate()}, ${d.getFullYear()}`;
}

/** "SEP 05" — timeline gutter. */
export function formatGutter(iso: ISODate): { month: string; day: string } {
  const d = fromISODate(iso);
  return {
 ...
 */
export function countdownLabel(from: ISODate, to: ISODate): string {
...
export function durationTogether(since: ISODate, now: ISODate = today()): string {
...
export function dueDate(tier: RitualTier, lastCompleted: ISODate | null): ISODate {
