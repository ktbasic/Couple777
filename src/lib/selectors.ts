...
export function hasMatches(state: AppState) {
...
export function ideaMatches(state: AppState) {
...
export type NotificationKind =
  ...
  | 'from-partner'
  /* Both of you hearted the same idea without knowing. */
  | 'idea-match'
...
export function notifications(state: AppState, meId: ID, partnerId: ID, now = today()): AppNotification[] {
  ...
  for (const row of state.savedIdeas) {
    if (!row.matchedAt || row.matchSeen) continue;
    items.push({
      id: `match-${row.id}`,
      kind: 'idea-match',
      emoji: '💕',
