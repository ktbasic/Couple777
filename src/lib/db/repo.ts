...
export async function upsertMemoryRow(row: Partial<MemoryRow>, id?: string): Promise<MemoryRow> {
  const db = requireDb();
  const write = async (r: Partial<MemoryRow>): Promise<MemoryRow> =>
    id
  ...
  try {
     ...
     */
    if (!isMissingColumn(e, 'visibility')) throw e;
    if (row.visibility === 'private') {
      throw new Error(
        'Private memories need one more migration. Run supabase/migrations/0002_memory_visibility.sql in the SQL editor, then keep this again.',
      );
    }
